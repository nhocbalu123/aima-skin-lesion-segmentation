...
def restore_binary_mask_to_image_shape(
    mask: np.ndarray,
    image_path: Path | str,
) -> np.ndarray:
    """Resize a model-space binary mask back to the source image dimensions."""

    source = Path(image_path)
    reference = cv2.imread(str(source), cv2.IMREAD_UNCHANGED)
    if reference is None:
        raise IOError(f"Could not read original image dimensions: {source}")
